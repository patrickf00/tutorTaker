...
struct Person
{
    int id;//will be representative of their profile number (easier to keep track)
    string subjects[10];
...
class Graph
{
    ...
    void addVertex(string name,bool isTutor, string school, string subject[10], int year, vector<double> ratings);//will create new Person vertex -add name to names vector
    ...
    double getRatingAverage(vector<double> ratings);
    string getYear(int year);
    void displaySubjects(string subject[]);
    bool isMatch(Person v1, Person v2);
    void displayMatches(Person v1);
